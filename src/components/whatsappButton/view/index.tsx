    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-80 h-[450px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div ref={chatRef} className="flex-1 p-3 space-y-2 overflow-y-auto bg-gray-50">
            {messages.map((msg, i) => (
              <div key={i} className={`max-w-[75%] px-3 py-2 rounded-lg text-sm ${
                msg.from === "bot" ? "bg-white" : "bg-[#DCF8C6] ml-auto"
              }`}>
                {msg.text}
              </div>
            ))}
