...
export default async function InternetGamersPage(props: { params: Promise<{ lang: string }> }) {
  const params = await props.params;
  const lang = params.lang as Locale;
  const dict = await getDictionary(lang);
  
  const post = dict.blogPosts.blogPost4;
    ...
    <>
      <section className="pt-28 pb-16 bg-white">
        <div className="container mx-auto px-6 max-w-3xl">
          ...
          <YouTubePlayer
            thumbnailSrc={dict.newLayout.blogPost4Img}
            videoId="YGAk_t2U3LE"
            alt={post.title}
          />
          ...
          <div className="prose prose-slate max-w-none text-[14px] md:text-[15px] leading-relaxed">
            ...
            <p className="mt-4">{post.p3}</p>
            <p className="mt-4">{post.p4}</p>
            ...
            <p>{post.sub5p1}</p>
            <p className="mt-4">{post.sub5p2}</p>
          </div>
        </div>
      </section>
